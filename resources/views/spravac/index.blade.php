...
<div class="row">	
	...
	<table class="table table-striped table-bordered table-condensed table-hover">
		...
		<tr>
			...
			<td>{{$spra->tecnico}}</td>
			<td>{{$spra->compania}}</td>
			<td>{{$spra->direccion_compania}}</td>
			<td>{{$spra->codigo_maquina}}</td>
			<td>{{$spra->jefe_planta}}</td>
			<td>   
			<a class="btn btn-success" title="PDF" href="MantenimientoSpravac_PDF/{{$spra->id}}" target="_blank"><i class="fa fa-file" aria-hidden="true"></i></a> 
			</td>
			</tr>
		@endforeach
		</tbody>
	...
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="pull-left" id="div_render">{!!$spravac->render()!!}  </div>
	</div>
</div>
   {!!Html::script('js/spravac.js')!!}
@endsection
